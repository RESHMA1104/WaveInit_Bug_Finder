...
Then(
    "the learner should be successfully signed-in to the dashboard",
    async function (this: BugFinder) {

        const expected = loginData.loginData[0].successMsg;

        logger.info(
            `Validating learner dashboard success message. Expected: ${expected}`
        );

        const actual =
            await this.learnerdashboardpage.getLearnerDashboardSuccessText();
        ...
        logger.info(
            "Learner successfully signed in and dashboard validation passed"
        );
    }
);

When(
    'the learner enter the username as {string}',
    async function (this: BugFinder, email) {
...
When(
    'the learner enter the password as {string}',
    async function (this: BugFinder, password) {
...
Then(
    'the learner should be displayed with an error message as {string}',
    async function (this: BugFinder, invalidLoginMessage) {

        logger.info(
            `Validating invalid login error message. Expected: ${invalidLoginMessage}`
        );

        const actual =
            await this.signinpage.getLearnersInvalidLoginMsg();
